...
def busqueda_archivo_sable(raiz, buscado, archivo):
    if(raiz is not None):
        x = leer(archivo, raiz.nrr)
        if(x.color_sable == buscado):
            print('------------')
            print('Nombre:' + str(x.nombre) + '\nEspecie: ' + str(x.especie) + '\nAño de nacimiento: ' + str(x.anio) + '\nColor del sable: ' + str(x.color_sable) + '\nRanking: ' + str(x.ranking) + '\nMaestro: ' +str(x.maestro))
            print('------------')
        busqueda_archivo_sable(raiz.izq, buscado, archivo)
        busqueda_archivo_sable(raiz.der, buscado, archivo)
