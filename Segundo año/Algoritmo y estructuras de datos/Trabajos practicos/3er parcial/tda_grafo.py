from tda_pila_dinamica import Pila, apilar, pila_vacia, desapilar
from tda_cola_dinamico import Cola, cola_vacia, arribo, atencion
from TDA_Heap import Heap, arribo_heap, atencion_heap, busqueda_heap_aero, busqueda_heap_antena, heap_vacio, cambiar_prioridad, busqueda_heap, busqueda_heap_red
...
def insertar_vertice_puestos(grafo, dato):
    '''Inserta un vértice al grafo aeropuerto'''
    nodo = Nodo_Vertice(dato)
    if grafo.inicio is None or grafo.inicio.info.nombre > dato.nombre:
        nodo.sig = grafo.inicio
...
def insertar_vertice_antenas(grafo, dato):
    '''Inserta un vértice al grafo aeropuerto'''
    nodo = Nodo_Vertice(dato)
    if grafo.inicio is None or grafo.inicio.info.ubicacion > dato.ubicacion:
        nodo.sig = grafo.inicio
        grafo.inicio = nodo
    else:
        ant = grafo.inicio
        act = grafo.inicio.sig
        while act is not None and act.info.ubicacion < nodo.info.ubicacion:
            ant = act
            act = act.sig
        nodo.sig = act
        ant.sig = nodo
...
def insertar_vertice_templos(grafo, dato):
    '''Inserta un vértice al grafo templos'''
    nodo = Nodo_Vertice(dato)
    if grafo.inicio is None or grafo.inicio.info.nombre > dato.nombre:
        nodo.sig = grafo.inicio
        grafo.inicio = nodo
...
def insertar_arista_puestos(grafo, dato, origen, destino):
    '''Inserta una arista desde el vértice origen al destino'''
    agregrar_arista_puestos(origen.adyacentes, dato, destino.info.nombre)
    if not grafo.dirigido:
        agregrar_arista_puestos(destino.adyacentes, dato, origen.info.nombre)

def insertar_arista_antenas(grafo, dato, origen, destino):
    '''Inserta una arista desde el vértice origen al destino'''
    agregrar_arista_viaje(origen.adyacentes, dato, destino.info.ubicacion)
    if not grafo.dirigido:
        agregrar_arista_viaje(destino.adyacentes, dato, origen.info.ubicacion)

def insertar_arista_viaje(grafo, dato, origen, destino):
    '''Inserta una arista desde el vértice origen al destino'''
    agregrar_arista_viaje(origen.adyacentes, dato, destino.info.cod)
    if not grafo.dirigido:
        agregrar_arista_viaje(destino.adyacentes, dato, origen.info.cod)

def insertar_arista_templo(grafo, dato, origen, destino):
    '''Inserta una arista desde el vértice origen al destino'''
    agregrar_arista_templo(origen.adyacentes, dato, destino.info.cod)
    if not grafo.dirigido:
        agregrar_arista_templo(destino.adyacentes, dato, origen.info.cod)


def agregrar_arista_templo(origen, dato, destino):
    '''Agrega la arista desde el vértice origen al destino'''
    nodo = Nodo_Arista(dato, destino)
    if origen.inicio is None or origen.inicio.destino > destino:
        nodo.sig = origen.inicio
...
def agregrar_arista_puestos(origen, dato, destino):
    '''Agrega la arista desde el vértice origen al destino'''
    nodo = Nodo_Arista(dato, destino)
    if origen.inicio is None or origen.inicio.destino > destino:
        nodo.sig = origen.inicio
...
def barrido_grafo_antena(grafo):
    ...
    aux = grafo.inicio
    while aux is not None:
        print('vertice:', aux.info.ubicacion)
        print('adyacentes:')
        adyacentes_red(aux)
        print()
        aux = aux.sig
...
def buscar_vertice_templos(grafo, buscado):
    '''Devuelve la direccion del elemento buscado'''
    aux = grafo.inicio
    while aux is not None and aux.info.nombre != buscado:
        aux = aux.sig
...
def buscar_vertice_puestos(grafo, buscado):
    '''Devuelve la direccion del elemento buscado'''
    aux = grafo.inicio
    while aux is not None and aux.info.nombre != buscado:
        aux = aux.sig
...
def buscar_vertice_antenas(grafo, buscado):
    '''Devuelve la direccion del elemento buscado'''
    aux = grafo.inicio
...
def buscar_vertice_antenas_codigo(grafo, buscado):
    aux = grafo.inicio
    while aux is not None and aux.info.cod != buscado:
        aux = aux.sig
    return aux
...
def dijkstra_antena(grafo, origen, destino):
    '''Dijkstra para hallar el camino mas corto'''
    no_visitados = Heap(tamanio_grafo(grafo))
    camino = Pila()
    aux = grafo.inicio
    while aux is not None:
        if aux.info.ubicacion == origen:
            arribo_heap(no_visitados, [aux, None], 0)
        else:
            arribo_heap(no_visitados, [aux, None], inf)
        aux = aux.sig
    while not heap_vacio(no_visitados):
        ...
        while aux is not None:
            ...
            if no_visitados.vector[pos][0] > dato[0] + aux.info:
                no_visitados.vector[pos][1][1] = dato[1][0].info.ubicacion
                cambiar_prioridad(no_visitados, pos, dato[0] + aux.info)
            aux = aux.sig
    return camino
...
def prim_puesto(grafo):
    '''Algoritmo de Prim para hallar el árbol de expansión mínimo'''
    bosque = []
    aristas = Heap(tamanio_grafo(grafo) ** 2)
    adyac = grafo.inicio.adyacentes.inicio
    while adyac is not None:
        arribo_heap(aristas, [grafo.inicio.info.costo, adyac.destino], adyac.info)
        adyac = adyac.sig
    while len(bosque) // 2 < tamanio_grafo(grafo) and not heap_vacio(aristas):
        dato = atencion_heap(aristas)
        if len(bosque) == 0 or ((dato[1][0] not in bosque) ^ (dato[1][1] not in bosque)):
            bosque += dato[1]
            destino = buscar_vertice_puestos(grafo, dato[1][1])
            adyac = destino.adyacentes.inicio
            while adyac is not None:
                arribo_heap(aristas, [destino.info, adyac.destino], adyac.info)
                adyac = adyac.sig
