...
def busqueda_proximidad_criaturas(raiz, buscado):
    if(raiz is not None):
        if(raiz.info[0:len(buscado)] == buscado):
            print('Nombre: ',raiz.info)
            print('Derrotado por: ',raiz.nrr)
        busqueda_proximidad_criaturas(raiz.izq, buscado)
        busqueda_proximidad_criaturas(raiz.der, buscado)
